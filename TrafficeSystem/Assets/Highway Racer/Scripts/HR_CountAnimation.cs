...
public class HR_CountAnimation : MonoBehaviour {
	...
	public void Count () {
		...
		if(GameObject.Find(HighwayRacerProperties.Instance.countingPointsAudioClip.name))
			countingAudioSource = GameObject.Find(HighwayRacerProperties.Instance.countingPointsAudioClip.name).GetComponent<AudioSource>();
		else
			countingAudioSource = CreateAudioSource.NewAudioSource(Camera.main.gameObject, HighwayRacerProperties.Instance.countingPointsAudioClip.name, 0f, 0f, 1f, HighwayRacerProperties.Instance.countingPointsAudioClip, true, true, true );
