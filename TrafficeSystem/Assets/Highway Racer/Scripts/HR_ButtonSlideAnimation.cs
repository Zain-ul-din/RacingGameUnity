...
public class HR_ButtonSlideAnimation : MonoBehaviour {
	...
	public void Animate () {
		...
		if(GameObject.Find(HighwayRacerProperties.Instance.labelSlideAudioClip.name))
			slidingAudioSource = GameObject.Find(HighwayRacerProperties.Instance.labelSlideAudioClip.name).GetComponent<AudioSource>();
		else
			slidingAudioSource = CreateAudioSource.NewAudioSource(Camera.main.gameObject, HighwayRacerProperties.Instance.labelSlideAudioClip.name, 0f, 0f, 1f, HighwayRacerProperties.Instance.labelSlideAudioClip, false, false, true );
