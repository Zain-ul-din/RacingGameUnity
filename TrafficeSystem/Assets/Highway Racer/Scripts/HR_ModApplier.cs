...
public class HR_ModApplier : MonoBehaviour {
	...
	void Awake () {
		...
		if(getColor)
			bodyColor = PlayerPrefsX.GetColor(transform.name + "BodyColor", HighwayRacerProperties.Instance._defaultBodyColor);
    ...
    void CreateSiren(){
		...
		GameObject frontSiren = (GameObject)Instantiate(HighwayRacerProperties.Instance.attachableSiren, GetComponent<RCC_CarControllerV3>().transform.position, transform.rotation);
		GameObject rearSiren = (GameObject)Instantiate(HighwayRacerProperties.Instance.attachableSiren, GetComponent<RCC_CarControllerV3>().transform.position, transform.rotation * Quaternion.Euler(0f, 180f, 0f));
