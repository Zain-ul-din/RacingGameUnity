...
public class HR_MainMenuHandler : MonoBehaviour {
	...
	void Awake(){
		...
		if (HighwayRacerProperties.Instance.mainMenuClips != null && HighwayRacerProperties.Instance.mainMenuClips.Length > 0) {
			
			mainMenuSoundtrack = CreateAudioSource.NewAudioSource (gameObject, "Main Menu Soundtrack", 0f, 0f, 1f, HighwayRacerProperties.Instance.mainMenuClips [UnityEngine.Random.Range (0, HighwayRacerProperties.Instance.mainMenuClips.Length)], true, true, false);
			mainMenuSoundtrack.ignoreListenerPause = true;
			mainMenuSoundtrack.ignoreListenerVolume = true;

		}

		if(HighwayRacerProperties.Instance._1MMoneyForTesting)
