...
public class HR_PlayerHandler : MonoBehaviour {
	...
	private int minimumSpeedForGainScore
	{
		get
		{
			return HighwayRacerProperties.Instance._minimumSpeedForGainScore;
		}
	}
	private int minimumSpeedForHighSpeed
	{
		get
		{
			return HighwayRacerProperties.Instance._minimumSpeedForHighSpeed;
		}
	}
	...
	void OnEnable(){
		...
		hornSource = CreateAudioSource.NewAudioSource(gameObject, "Horn", 10f, 100f, 1f, HighwayRacerProperties.Instance.hornClip, true, false, false);
	...
	void Update () {
		...
		if(HR_GamePlayHandler.Instance.mode == HR_GamePlayHandler.Mode.Bomb){
			...
			if(bombHealth <= 0f){
				GameObject explosion = (GameObject)Instantiate(HighwayRacerProperties.Instance.explosionEffect, transform.position, transform.rotation);
	...
	void CheckNearMiss(){
		...
		if(Physics.Raycast(carController.COM.position, (-transform.right), out hit, 2f, HighwayRacerProperties.Instance.trafficCarsLayer) && !hit.collider.isTrigger){

			currentTrafficCarNameLeft = hit.transform.name;

		}else{
			
			if(currentTrafficCarNameLeft != null && speed > HighwayRacerProperties.Instance._minimumSpeedForGainScore){
		...
		if(Physics.Raycast(carController.COM.position, (transform.right), out hit, 2f, HighwayRacerProperties.Instance.trafficCarsLayer) && !hit.collider.isTrigger){
			...
			if(currentTrafficCarNameRight != null && speed > HighwayRacerProperties.Instance._minimumSpeedForGainScore){
		...
		if(Physics.Raycast(carController.COM.position, (transform.forward), out hit, 40f, HighwayRacerProperties.Instance.trafficCarsLayer) && !hit.collider.isTrigger){
		...
		if (hornSource) {
			...
			if (carController.highBeamHeadLightsOn) {
				...
				if (GetComponent<HR_ModApplier>().isSirenAttached)
					hornSource.clip = HighwayRacerProperties.Instance.sirenAudioClip;
	...
	void OnCollisionEnter(Collision col){
		...
		if (colRelVel.magnitude * cos < HighwayRacerProperties.Instance._minimumCollisionForGameOver && colRelVel.magnitude * cos2 < HighwayRacerProperties.Instance._minimumCollisionForGameOver)
			return;

		combo = 0;

		if(col.relativeVelocity.magnitude < HighwayRacerProperties.Instance._minimumCollisionForGameOver || (1 << col.gameObject.layer) != HighwayRacerProperties.Instance.trafficCarsLayer.value)
			return;
